...
public class AiStateIdle implements AiState {
	private AiStateManager aiStateManager;
	private AI_STATE state = AI_STATE.IDLE;

	public AiStateIdle(AiStateManager aiStateManager) {
		this.aiStateManager = aiStateManager;
	}

	@Override
	public void entry() {
		MusicManager.getInstance().sendCommand(AudioCommand.SOUND_PLAY_ONCE, AudioEnum.forName(String.format("SOUND_IDLE_STATE_%s", aiStateManager.getEnemy().getName().toUpperCase())));
	}
