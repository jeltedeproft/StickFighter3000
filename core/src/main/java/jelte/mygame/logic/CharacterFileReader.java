...
public class CharacterFileReader {
	...
	public static void loadUnitStatsInMemory() {
		if (!statsLoaded) {
			...
			try {
				unitStats = (ArrayList<CharacterData>) json.fromJson(ClassReflection.forName("java.util.ArrayList"), ClassReflection.forName("jelte.mygame.logic.CharacterData"), Gdx.files.internal(Constants.UNIT_STATS_FILE_LOCATION));
