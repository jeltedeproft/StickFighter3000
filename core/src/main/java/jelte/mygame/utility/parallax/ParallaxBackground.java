...
import jelte.mygame.utility.parallax.ParallaxLayer.TileMode;

/**
 * A ParallaxBacground encapsulates the logic to Render ParallaxLayers. It owns a collection of ParallaxLayers. These Layers a rendered on screen
 * showing parallax effect based on scrolling characteristics of individual layers. This rendering module does not owns any Batch ,you need to provide
...
public class ParallaxBackground {
	...
	/**
	 * render the layers held by this module. Of course the layers are rendered in parallax scrolling manner. The worldCamera and batch provided are
	 * unaffected by the method
	 *
	...
	public void draw(OrthographicCamera worldCamera, Batch batch) {
		...
		for (int i = 0; i < layers.size; i++) {
			final ParallaxLayer layer = layers.get(i);
			final Vector2 origCameraPos = new Vector2(cachedPos.x, cachedPos.y);
			worldCamera.position.set(origCameraPos.scl(layer.getParallaxRatio()), cachedPos.z);
			worldCamera.update();
			batch.setProjectionMatrix(worldCamera.combined);
			float currentX = (layer.getTileModeX().equals(TileMode.SINGLE) ? 0
					: (int) ((worldCamera.position.x - worldCamera.viewportWidth * .5f * worldCamera.zoom) / layer.getWidth()) * layer.getWidth())
					- Math.abs((1 - layer.getParallaxRatio().x) % 1) * worldCamera.viewportWidth * .5f;
			do {
				float currentY = (layer.getTileModeY().equals(TileMode.SINGLE) ? 0
						: (int) ((worldCamera.position.y - worldCamera.viewportHeight * .5f * worldCamera.zoom) / layer.getHeight()) * layer.getHeight())
						- (1 - layer.getParallaxRatio().y) % 1 * worldCamera.viewportHeight * .5f;
				do {
