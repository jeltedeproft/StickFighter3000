...
/**
 *
 * TextureRegionParallaxLayer is the direct extension of {@link ParallaxLayer}. It represents a layer in the parallax scrolling background with just
 * one texture region per layer . It still is very nifty and useful as it encapsulates the logic of padding in all the directions. The padding can
 * help you avoid using single full screen texture regions of background.
 * <p>
 * As an example let us suppose you wish to render repeatedly a single cloud region with 1/10 the width and height of the screen . You wish to render
 * it at half way above the screen baseline. An plus no more than one could should be there within one screen width. You can easily achieve this using
 * proper bottom, left and right padding in this layer
 *
 * @author Rahul
 *
 */
public class TextureRegionParallaxLayer extends ParallaxLayer {
	...
	/**
	 * Creates a TextureRegionParallaxLayer with either regionWidth or regionHeight equal oneDimen specified, while the other is calculated maintaining
	 * the aspect ratio of the region. Paddings are set to 0.
	 *
	 ...
	 * <p>
	 * You might be wondering that why are topPadding and rightPadding not used , what is their use then . Well they are used by ParallaxBackground when
	 * it renders this layer . During rendering it pings the {@link #getWidth()}/{@link #getHeight()} method of this layer which in
	 * {@link TextureRegionParallaxLayer} implementation return the sum of regionWidth/regionHeight and paddings.
	 */
	@Override
