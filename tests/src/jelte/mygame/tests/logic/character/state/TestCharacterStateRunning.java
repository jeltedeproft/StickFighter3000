...
public class TestCharacterStateRunning {

	private CharacterStateManager characterStateManager;
	private CharacterStateRunning characterState;
	...
	@Test
	public void testHandleEventLeftUnPressedStandingStill() {
		when(characterStateManager.characterisStandingStill()).thenReturn(true);
		EVENT event = EVENT.LEFT_UNPRESSED;
		characterState.handleEvent(event);

		verify(characterStateManager).transition(CHARACTER_STATE.STOPRUNNING);
	}
	...
	@Test
	public void testHandleEventRightUnPressedStandingStill() {
		when(characterStateManager.characterisStandingStill()).thenReturn(true);
		EVENT event = EVENT.RIGHT_UNPRESSED;
		characterState.handleEvent(event);
	...
	@Test
	public void testHandleEventSprinting() {
		EVENT event = EVENT.SPRINT_PRESSED;
		characterState.handleEvent(event);

		verify(characterStateManager).transition(CHARACTER_STATE.SPRINTING);
	}
