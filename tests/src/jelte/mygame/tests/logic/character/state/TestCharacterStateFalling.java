...
public class TestCharacterStateFalling {

	private CharacterStateManager characterStateManager;
	private CharacterStateFalling characterState;
	...
	@Test
	public void testUpdateCollisionLandedGround() {
		...
		verify(characterStateManager).pushState(CHARACTER_STATE.LANDING);
	}

	@Test
	public void testUpdateCollisionLandedPlatform() {
		when(characterStateManager.getCharacterCollisions()).thenReturn(new Array<>(new Collidable.COLLIDABLE_TYPE[] { Collidable.COLLIDABLE_TYPE.STATIC_PLATFORM}));
		when(characterStateManager.characterIsFalltrough()).thenReturn(false);
		float delta = 1f;
	...
	public void testHandleEventAttackPressed() {
		...
		verify(characterStateManager).pushState(CHARACTER_STATE.FALLATTACKING);
	}
